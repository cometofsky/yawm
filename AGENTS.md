...
- Tests: `npm test` (runs `scripts/verify-umalqura.js` <!-- claim: cmd=`grep '"test":' package.json` expect=`"test": "node scripts/verify-umalqura.js"` --> — verifies the embedded Umm al-Qura table against Intl's islamic-umalqura calendar; exits non-zero on any mismatch)
